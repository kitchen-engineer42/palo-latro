...
-- Mirrors Balatro's "everyone reaches into one well-known global" style.
...
G.STATES = {
  ...
  BLIND_SELECT   = 9,   -- pre-blind preview/select: kind/target/reward + boss telegraph
  USE_CARD       = 10,  -- a consumable is resolving (transient; gates input)
  TARGET_SELECT  = 11,  -- a targeted consumable awaits the player's card pick(s)
...
-- This flat pool remains efficient with many on-screen objects.
...
-- Clocks (the runtime design). TOTAL (gameplay, speed-scaled/pausable), REAL (wall-clock UI), and
-- BACKGROUND is wall-clock shader time; it drives the animated backdrop and edition shimmer.
...
G.SETTINGS = { gamespeed = 4, paused = false, reduced_motion = false, sound = true, shaders = true, crt = false }
...
-- Resolution independence: the whole frame is laid out + drawn at a fixed VIRTUAL resolution
...
-- Virtual design resolution mirrors Balatro's room: 20 × 11.5 tiles. G.TILE = px per tile in virtual
...
G.CANVAS = nil                    -- native-resolution canvas for the optional post-processing pass
G.SHADERS = {}                    -- name → compiled love.Shader, auto-loaded at boot from assets/shaders/*.glsl
...
-- are GLSL effects active right now? (master setting on · motion not reduced · registry non-empty).
...
function shaders_enabled()
  return G.SETTINGS.shaders and not G.SETTINGS.reduced_motion and next(G.SHADERS or {}) ~= nil
end
