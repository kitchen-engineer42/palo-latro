...
function Bosses.reliability_penalty(key, cards, context)
  ...
  local penalty = 0
  local analysis = (context and context.coverage) or Coverage.analyze(cards or {})
  if b.kind == "reliability" and b.layer then
    for _, c in ipairs(cards or {}) do if Coverage.card_has_layer(c, b.layer, analysis) then penalty = penalty + (b.reliability or 0) end end
  elseif b.kind == "role" then
...
function Bosses.score_multiplier(key, cards, context)
  ...
  if b.kind == "layer" and b.layer then
    local analysis = (context and context.coverage) or Coverage.analyze(cards or {})
    for _, card in ipairs(cards or {}) do if Coverage.card_has_layer(card, b.layer, analysis) then return b.revenue_mult or 1 end end
